...
<head>
	...
	<title>Flora Lins Encomendas</title>
</head>
<body class="container">
	...
	<h3 class="text-primary">Encomenda de <b><?php echo $_SESSION['usuario']; ?></b> - 
	  ...
	  <div class="modal-dialog modal-dialog-centered" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h5 class="modal-title" id="formModalCenterTitle">Nova Encomenda</h5>
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
	          <span aria-hidden="true">&times;</span>
	        </button>
	      </div>
	      <div class="modal-body">
	        <?php include 'form_encomenda.php'; ?>
	      </div>
	    </div>
	  </div>
	</div>
	...
	{
		$sql = "SELECT id_flor, nome, preco, quantidade FROM
			t_flor INNER JOIN tb_usuario ON t_flor.id_usuario = tb_usuario.id_usuario WHERE t_flor.id_usuario = $id_usuario";
	}
	...
	{
		$encomendas = array(); // prepara um array 
		...
		while ($encomendas = mysqli_fetch_assoc($resultado)) 
		{
			...
			echo '<td class="text-center"><a href="editar.php?id_flor='.$id_flor.'" class="btn btn-warning">Editar</a></td>';

			echo '<td class="text-center"><a href="deletar.php?id_flor='.$id_flor.'" class="btn btn-danger" onClick="return confirm(\'Deseja excluir este registro?\')">Deletar</a>';
			
			echo ' <input type="checkbox" name="selecionado[]" value="'.$id_flor.'"></td>';
	...
	{
		echo '<h3 class="alert alert-info">Não há encomendas cadastradas</h3>';
	}
