...
<body class="container">
	...
	<h3 class="text-dark">Playlist de <b><?php echo $_SESSION['usuario']; ?></b> - 
	...
	$sql = "SELECT id_musica, musica, banda, genero, tempo FROM
			tb_musica INNER JOIN tb_usuario ON tb_musica.id_usuario = tb_usuario.id_usuario WHERE tb_musica.id_usuario = $id_usuario ";
	}
	...
	if(mysqli_affected_rows($conn) > 0)
	{
		...
		while ($musicas = mysqli_fetch_assoc($resultado)) 
		{
			...
			foreach ($musicas as $coluna_atual => $valor_atual) 
			{
				...
				/*
				else if($coluna_atual == 'tempo')
				{
					$valor_atual = datetime->format("H:i:s" = ($valor_atual));
				}
					*/
			...
			echo '<td class="text-center"><a href="editar.php?id_musica='.$id_musica.'" class="btn btn-outline-success">Editar</a></td>';

			echo '<td class="text-center"><a href="deletar.php?id_musica='.$id_musica.'" class="btn btn-outline-danger" onClick="return confirm(\'Deseja excluir este registro?\')">Deletar</a>';
		...
		echo '<button type="submit" class="btn btn-outline-danger" onClick="return confirm (\'Deseja excluir todos os selecionados?\')">
